...
func Build(plan *parser.TerraformPlan) *graph.Graph {
	...
	nodeMap, nodeKeys := buildNodeLookup(g.Nodes)
...
// buildNodeLookup creates a map and sorted keys for efficient node lookup.
// Keys are sorted by length (descending) to ensure longest matches are found first.
func buildNodeLookup(nodes []graph.Node) (map[string]graph.Node, []string) {
...
// Returns empty string if the reference is not a resource (e.g., var, local).
